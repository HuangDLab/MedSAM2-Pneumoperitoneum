...
# ---------------------------------------------------------------------------
# MODIFICATIONS
# This file has been modified from the original MedSAM2 / SAM 2 source.
# Changes made in this work:
#   * inject LoRA adapters into the image encoder trunk and the SAM mask decoder,
#     controlled by a new `custom_config` block in the training config
#   * load the pretrained checkpoint before LoRA injection, then disable the
#     built-in initializer so it is not applied a second time
#   * freeze the base model and keep the memory modules trainable in LoRA mode
#   * unfreeze the mask decoder for full fine-tuning when encoder LoRA is used
#     without decoder LoRA
#   * accept a `custom_config` argument on Trainer
# Original source: https://github.com/bowang-lab/MedSAM2 (Apache-2.0)
# ---------------------------------------------------------------------------
...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        # === [added] LoRA switches, supplied by the training config ===
    ...
    def _setup_components(self):
        ...
        # ============================================================
        # [added] Load the pretrained checkpoint BEFORE injecting LoRA.
        # ============================================================
        # The checkpoint has the standard architecture, so it has to be loaded
        # while the model still has the standard architecture as well.
        if self.checkpoint_conf.model_weight_initializer is not None:
            logging.info("Pre-loading standard checkpoint BEFORE LoRA injection...")
            try:
                # run the configured initializer manually
                initializer = instantiate(self.checkpoint_conf.model_weight_initializer)
                self.model = initializer(model=self.model)
                logging.info("✅ Standard checkpoint loaded successfully.")
                
                # Clear the initializer afterwards, otherwise the Trainer would
                # later try to load the standard checkpoint into the LoRA-wrapped
                # model and fail.
                self.checkpoint_conf.model_weight_initializer = None
            except Exception as e:
                # Warning only: check the log for "Standard checkpoint loaded
                # successfully" to confirm the weights were applied.
                logging.warning(f"Failed to pre-load checkpoint: {e}")
                logging.warning("Will attempt standard loading later (this might fail with LoRA).")
        # ============================================================
        
        # === [added] LoRA injection, driven by custom_config ===
        use_lora_enc = self.custom_config.get("enable_encoder_lora", False)
        use_lora_dec = self.custom_config.get("enable_decoder_lora", False)
        lora_rank = self.custom_config.get("lora_rank", 32)

        # If either LoRA is enabled, freeze everything first, then unfreeze selectively.
        if use_lora_enc or use_lora_dec:
            logging.info("LoRA mode detected. Freezing base model parameters.")
            for param in self.model.parameters():
                param.requires_grad = False
            
            # Memory modules stay trainable to preserve cross-slice (3D) context.
            logging.info("Unfreezing Memory modules for 3D context.")
            for param in self.model.memory_attention.parameters(): param.requires_grad = True
            for param in self.model.memory_encoder.parameters(): param.requires_grad = True

        # Inject Encoder LoRA
        if use_lora_enc:
            logging.info(f"Injecting LoRA (Rank {lora_rank}) into Image Encoder.")
            enc_cfg = LoraConfig(r=lora_rank, lora_alpha=lora_rank*2, target_modules=["qkv", "proj"], lora_dropout=0.05, bias="none")
            self.model.image_encoder.trunk = get_peft_model(self.model.image_encoder.trunk, enc_cfg)

        # Inject Decoder LoRA
        if use_lora_dec:
            logging.info(f"Injecting LoRA (Rank {lora_rank}) into Mask Decoder.")
            dec_cfg = LoraConfig(r=lora_rank, lora_alpha=lora_rank*2, target_modules=["q_proj", "k_proj", "v_proj", "out_proj"], lora_dropout=0.05, bias="none")
            self.model.sam_mask_decoder = get_peft_model(self.model.sam_mask_decoder, dec_cfg)
        
        # Encoder LoRA without decoder LoRA: train the decoder with full parameters.
        if use_lora_enc and not use_lora_dec:
            logging.info("Unfreezing Mask Decoder for full fine-tuning.")
        ...
        self.loss = None
        if self.loss_conf:
            self.loss = {key: instantiate(el, _convert_="all") for key, el in self.loss_conf.items()}
